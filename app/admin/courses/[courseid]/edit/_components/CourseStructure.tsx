...
export function CourseStructure({ data }: iAppProps) {
  const initialItems =
    data.chapter.map((chapter) => ({
      ...
      isOpen: true, // deafult chapters to open
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    if (activeType === "chapter") {
      ...
      if (oldIndex === -1 || newIndex === -1) {
        toast.error("Could not find chapter old/new index for reording");
        return;
      }
      ...
      if (courseId) {
        ...
        toast.promise(reorderPromise(), {
          ...
          error: () => {
            ...
            return "Faild too reorder chapters";
          },
        });
      }
      return;
    ...
    if (activeType === "lesson" && overType === "lesson") {
      ...
      if (courseId) {
        ...
        const reorderLessonsProomise = () =>
          reorderLessons(chapterId, lessonsToUpdate, courseId);
        toast.promise(reorderLessonsProomise(), {
          ...
          error: () => {
            setItems(previousItems);
            return "Faild to reorder lessons";
          },
        });
      }
      return;
